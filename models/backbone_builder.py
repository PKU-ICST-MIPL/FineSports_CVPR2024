...
from models.backbones.video_swin_transformer import build_swin
from .I3D_Backbone import I3D_backbone
...
class Backbone(nn.Module):
    ...
    def __init__(self, train_backbone: bool, num_channels: int, position_embedding, cfg):
        ...
        elif cfg.CONFIG.MODEL.BACKBONE_NAME == 'I3D':
            print('I3D backbone')
            self.body = build_I3D(cfg)
        elif cfg.CONFIG.MODEL.BACKBONE_NAME == 'SWIN':
            print('Video SWIN backbone')
            self.body = build_swin(cfg)
        else:
        ...
        if cfg.CONFIG.MODEL.SINGLE_FRAME:
            if cfg.CONFIG.MODEL.TEMPORAL_DS_STRATEGY == 'avg':
                ...
                self.query_pool = nn.Embedding(1, num_channels)
                self.pool_decoder = LSTRTransformerDecoder(
                    LSTRTransformerDecoderLayer(d_model=num_channels, nhead=8, dim_feedforward=num_channels, dropout=0.1), 1,
                    norm=layer_norm(d_model=num_channels, condition=True))
