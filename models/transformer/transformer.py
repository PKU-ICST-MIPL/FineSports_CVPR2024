...
class Transformer(nn.Module):
    ...
    def remove_encoder_layers(self, layers_to_remove):
        self.encoder.remove_layers(layers_to_remove)
    
    def remove_decoder_layers(self, layers_to_remove):
        self.decoder.remove_layers(layers_to_remove)
    
    def clone_layers(self, layers_to_clone):
        print('cloning layers: ', layers_to_clone)
        self.encoder.layers.extend([copy.deepcopy(self.encoder.layers[i]) for i in layers_to_clone])
        self.encoder.num_layers = len(self.encoder.layers)
        self.decoder.layers.extend([copy.deepcopy(self.decoder.layers[i]) for i in layers_to_clone])
        self.decoder.num_layers = len(self.decoder.layers)
...
class TransformerEncoder(nn.Module):
    ...
    def remove_layers(self, layers_to_remove):
        print('removing encoder layers', layers_to_remove)
        self.layers = nn.ModuleList([layer for i, layer in enumerate(self.layers) if i not in layers_to_remove])
...
class TransformerDecoder(nn.Module):
    ...
    def remove_layers(self, layers_to_remove):
        print('removing decoder layers', layers_to_remove)
        self.layers = nn.ModuleList([layer for i, layer in enumerate(self.layers) if i not in layers_to_remove])
        self.num_layers = len(self.layers)
