...
import evaluates.utils.standard_fields as standard_fields
import evaluates.utils.label_map_util as label_map_util
import evaluates.utils.metrics as metrics
import evaluates.utils.per_image_evaluation as per_image_evaluation
...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    def evaluate(self):
        ...
        (per_class_ap, mean_ap, _, _, per_class_corloc, mean_corloc) = (
                self._evaluation.evaluate(self._categories))
        pascal_metrics = {
                self._metric_prefix +
        ...
        category_list = []
        for id in range(len(category_index)):
            category_list.append(category_index[id]['id'])
        for idx in range(per_class_ap.size):
            if idx + self._label_id_offset in category_list:
                display_name = (
                        self._metric_prefix + 'PerformanceByCategory/AP@{}IOU/{}'.format(
                                self._matching_iou_threshold,
                                category_index[category_list.index(idx + self._label_id_offset)]['name']))
...
class ObjectDetectionEvaluation(object):
    ...
    def add_single_ground_truth_image_info(self,
        ...
        if image_key in self.groundtruth_boxes:
            logging.warn(
                    ...
                    image_key)
            return
        # groundtruth_class_labels = np.ones(groundtruth_class_labels.shape)
    ...
    def add_single_detected_image_info(self,
        ...
        if image_key in self.detection_keys:
            logging.warn(
                    ...
                    image_key)
            return
        # detected_class_labels = np.ones(detected_class_labels.shape)
    ...
    def evaluate(self, cate):
        ...
        category_index = label_map_util.create_category_index(cate)
        for class_index in range(self.num_class):
            ...
            # cate_label = category_index[class_index + 1]['name']
            if 0:
                plt.plot(recall, precision, label=cate_label)
        ...
        if 0:
            plt.savefig('AP_Cate.png',bbox_inches='tight')
