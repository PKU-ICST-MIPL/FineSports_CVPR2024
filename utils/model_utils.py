...
def load_detr_weights(model, pretrain_dir, cfg):
    ...
    if not cfg.DDP_CONFIG.DISTRIBUTED:
...
def deploy_model(model, cfg):
    ...
    if cfg.DDP_CONFIG.DISTRIBUTED:
        if cfg.DDP_CONFIG.GPU is not None:
            ...
            model.text_fuser.to_cuda("cuda:"+str(cfg.DDP_CONFIG.GPU))
    ...
    model.module.transformer.clone_layers([5,5])
    return model


def load_model(model, cfg, optimizer, lr_scheduler, load_fc=True):
    """
    ...
    if os.path.isfile(cfg.CONFIG.MODEL.PRETRAINED_PATH):
        ...
        not_found_dict = {k[7:]: v for k, v in model_dict.items() if not k[7:] in checkpoint['model']}
...
def save_model(model, optimizer, lr_scheduler, epoch, cfg, model_ema=None):
    ...
    save_checkpoint(cfg, epoch, model, None, optimizer, lr_scheduler, filename=checkpoint, model_ema=model_ema)


def save_checkpoint(cfg, epoch, model, max_accuracy, optimizer, lr_scheduler, filename, model_ema=None):
